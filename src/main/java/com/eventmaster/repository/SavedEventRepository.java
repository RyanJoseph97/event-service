...
public interface SavedEventRepository extends JpaRepository<SavedEvent, Long> {
    ...
    @Query("SELECT se FROM SavedEvent se JOIN FETCH se.event WHERE se.username = :username")
    List<SavedEvent> findByUsernameWithEvent(@Param("username") String username);
